...
@dataclass(frozen=True)
class EnvConfig:
    do: Range = Range(1.5,9.5)
    ph: Range = Range(6.5, 9.0)
    nh3: Range = Range(0.0, 1.2)
    no2: Range = Range(0.0, 1.0)
    co2: Range = Range(0.0, 25.0)
    orp: Range = Range(150.0, 450.0)
    temp: Range = Range(22.0, 35.0)
...
class BehaviourConfig:
    ...
    crowding_index: Range = Range(0.1, 1.0)
...
class BiologicalConfig:
    cortisol: Range = Range(10.0, 300.0)
...
class HumanInfraConfig:
    ...
    ambient_noise_db: Range = Range(45.0, 100.0)


@dataclass(frozen=True)
class LifecycleConfig:
    mortality_events: Range = Range(0, 8)           # int range
    grading_events: Range = Range(0, 3)              # int range
    awg_grams: Range = Range(50.0, 700.0)
...
class AquacultureSyntheticGenerator:
    ...
    def generate(self, n_samples: Optional[int] = None) -> pd.DataFrame:
        ...
        stress += np.clip(5.0 - do, 0, 4) * 0.15
        stress += np.clip(nh3 - 0.1, 0, None) * 0.12
        stress += np.clip(no2 - 0.1, 0, None) * 0.08
        stress += np.clip(temp - 32.0, 0, None) * 0.08
        stress += np.clip(22.0 - temp, 0, None) * 0.05
        stress += np.clip(abs(ph - 7.5) - 0.8, 0, None) * 0.08
        ...
        stress += np.clip(cortisol - 30.0, 0, 50) * 0.003 # Mild cortisol elevation
        stress += np.clip(cortisol - 80.0, 0, None) * 0.01 #severe cortisol elevation
        stress += microbiome_dysbiosis * 0.2
        stress += np.clip(ambient_noise_db - 70.0, 0, None) * 0.01
        stress += human_presence_count * 0.03
        stress += vibration_level * 0.08
        #stress += mortality_events * 0.02
        stress += grading_events * 0.05
        stress += np.clip(stocking_density - 25.0, 0, None) * 0.03
        stress += np.where(weather == "stormy", 0.08, 0.0)
        stress += np.where(weather == "rainy", 0.03, 0.0)
        stress += np.where(system_type == "flow_through", 0.1, 0.0)
        stress += np.clip(500.0 - light_lux, 0, None) * 0.002  # NEW
    ...
    def generate_sequences(
        ...
        IDX_NO2 = NUMERIC_COLS.index("no2_mg_per_l")
        ...
        for _ in range(n_sequences):
            ...
            base_row[IDX_NH3] = np.clip(base_row[IDX_NH3], 0.0, 0.15)
            base_row[IDX_DO]  = np.clip(base_row[IDX_DO],  6.5, 9.5)
            base_row[IDX_TEMP]     = np.clip(base_row[IDX_TEMP],    28.0, 31.0)
            base_row[IDX_NO2] = np.clip(base_row[IDX_NO2], 0.0, 1.0)
            base_row[IDX_CORTISOL] = np.clip(base_row[IDX_CORTISOL], 5.0, 40.0)
            base_row[IDX_MICRO]    = np.clip(base_row[IDX_MICRO],    0.0,  0.2)
            base_row[IDX_CROWDING]    = np.clip(base_row[IDX_CROWDING],    0.1,  0.5)

            seq_X = []
            seq_y = []

            for _ in range(timesteps):
                drift    = np.random.normal(0, 0.05, size=base_row.shape)
                base_row = base_row + drift

                # Float clips
                base_row[IDX_DO]       = np.clip(base_row[IDX_DO],       1.0,   9.0)
                base_row[IDX_NH3]      = np.clip(base_row[IDX_NH3],      0.0,    1.2)
                base_row[IDX_NO2] = np.clip(base_row[IDX_NO2], 0.0, 1.0)
                base_row[IDX_TEMP]     = np.clip(base_row[IDX_TEMP],    22.0,   36.0)
                base_row[IDX_CROWDING] = np.clip(base_row[IDX_CROWDING], 0.1,    1.0)
                base_row[IDX_CORTISOL] = np.clip(base_row[IDX_CORTISOL], 5.0,  300.0)
                base_row[IDX_MICRO]    = np.clip(base_row[IDX_MICRO],    0.0,    1.0)
                base_row[IDX_LIGHT]    = np.clip(base_row[IDX_LIGHT],    0.0, 10000.0) 
                ...
                base_row[IDX_MORT]    = np.clip(round(base_row[IDX_MORT]),    0,  8)
                ...
                stress  = 0.0
                stress += np.clip(5.0 - base_row[IDX_DO], 0, 4) * 0.15
                stress += np.clip(base_row[IDX_NH3] - 0.1, 0, None) * 0.12
                stress += np.clip(base_row[IDX_NO2] - 0.1, 0, None) * 0.08
                stress += np.clip(base_row[IDX_TEMP] - 32.0, 0, None) * 0.08
                stress += np.clip(22.0 - base_row[IDX_TEMP], 0, None) * 0.05 
                stress += np.clip(base_row[IDX_CROWDING] - 0.8, 0, None) * 0.2
                stress += np.clip(base_row[IDX_CORTISOL] - 30.0, 0, 50) * 0.003
                stress += np.clip(base_row[IDX_CORTISOL] - 80.0, 0, None) * 0.01
                stress += base_row[IDX_MICRO] * 0.2
                stress += np.clip(base_row[IDX_LIGHT] - 5000.0, 0, None) * 0.0001
