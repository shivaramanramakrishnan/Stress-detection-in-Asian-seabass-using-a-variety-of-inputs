        ...
        trajectory.append(
            {
                ...
                "do_mg_per_l":  float(X[0, i, idx_do]),
                "temp_c":  float(X[0, i, idx_temp]),
                "nh3_mg_per_l":  float(X[0, i, idx_nh3]),
            }
        )
...
def synthetic_sample() -> Dict[str, Any]:
    ...
    key_features = {
        ...
        "fish_count": int(df["fish_count"].iloc[0]),
        "crowding_index": float(df["crowding_index"].iloc[0]),
        "mortality_events": int(df["mortality_events"].iloc[0]),
...
def _apply_simple_worsening_scenario(row: pd.Series, step_index: int) -> pd.Series:
    ...
    r["temp_c"]         = float(np.clip(r["temp_c"]         + 0.2 * t, 20.0, 34.0))
    r["nh3_mg_per_l"]  = float(np.clip(r["nh3_mg_per_l"]  + 0.03 * t,  0.0,  1.5))

    # Slightly increase crowding index to simulate stress from density / behaviour
    if "crowding_index" in r:
        r["crowding_index"] = float(np.clip(r["crowding_index"] + 0.02 * t,  0.1,  1.5))
...
def scenario_forecast(
    ...
    for _ in range(max_attempts):
        do_val   = df0["do_mg_per_l"].iloc[0]
        nh3_val  = df0["nh3_mg_per_l"].iloc[0]
        temp_val = df0["temp_c"].iloc[0]
        if do_val >= 5.0 and nh3_val <= 0.4 and temp_val <= 32.0:
            break
        df0 = gen.generate(1)
    base_row = df0.iloc[0]
